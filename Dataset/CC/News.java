...
/**
  * This class implements the interface for collecting news for the Operator Agency actor.
  *
  * @ Version 1.0
  * @ Author Mario Gallo
  *
...
// Setting up of the help manager to manage the news.
...
/*
* Inclusion of the frame on the desktop retrieves the desktop pane and desktop manager,
* And initializes the remote objects management.
*/
...
public void internalFrameOpened(InternalFrameEvent pEvent) {
    ...
    // Setting up objects for remote asset management Cultural.
    try {
        ...
        JOptionPane.showMessageDialog(desktopPane, error, "Error!", JOptionPane.ERROR_MESSAGE, err);
        frame.dispose();
    }
}
...
if (null == NewsToolbar)
{
NewsToolbar JToolBar = new NewsToolbar();
    ...
    if (null == btnModificaN) {
        ...
        btnModificaN.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent pActionEvent) {
                ...
                if (idNews == -1) { // Check if it's in edit mode
                ...
                Date expiryDate = (Date) tableModel.getValueAt(selectedRow, 3);
                durataNews.setSelectedIndex(Data.getNumDays(expiryDate));
    ...
    if (null == btnEliminaN) {
        ...
        btnEliminaN.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent pEvent) {
                ...
                // Construct the confirmation dialog for deletion
                JPanel root = new JPanel(new BorderLayout());
                JLabel message = new JLabel("Are you sure you want to delete the selected news?");
                message.setFont(new Font("Dialog", Font.BOLD, 14));
                JLabel notice = new JLabel("The deleted data cannot be recovered.", SwingConstants.CENTER);
                notice.setIcon(new ImageIcon(getClass().getResource(Home.URL_IMAGES + "warning16.png")));
                root.add(message, BorderLayout.NORTH);
                root.add(notice, BorderLayout.CENTER);
                String[] options = { "Delete", "Cancel" };

                // Show the dialog
                int choice = JOptionPane.showOptionDialog(jContentPane, root, "Confirm Delete News",
                        JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
                        new ImageIcon(getClass().getResource(Home.URL_IMAGES + "EliminaNews48.png")), options,
                        options[1]);
                ...
                if (choice == JOptionPane.YES_OPTION) {
                    try {
                        ...
                        JOptionPane.showInternalMessageDialog(jContentPane,
                                "The news has been successfully deleted.", "News Deleted", JOptionPane.OK_OPTION,
                        ...
                        ImageIcon err = new ImageIcon(
                                getClass().getResource(Home.URL_IMAGES + "error48.png"));
                        JOptionPane.showMessageDialog(jContentPane, error, "Error",
...
private JPanel getFormNews() {
    if (null == formNews) {
        ...
        formNews.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(new Color(51, 102, 255), 3),
                "New News", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), new Color(0, 102, 204)));
        GridBagConstraints g = new GridBagConstraints();
...
public void actionPerformed(ActionEvent pEvent) {
    if (testoNews.getText().length() == 0) {
        JOptionPane.showInternalMessageDialog(jContentPane,
                "The text of a news cannot be empty!",
    ...
    if (idNews != -1) { // We're making a change
        ...
        JLabel message = new JLabel(
                "Changing the selected news with new data?");
        message.setFont(new Font("Dialog", Font.BOLD, 14));
        JLabel alert = new JLabel(
                "The previous data cannot be recovered.");
        ...
        int choice = JOptionPane.showInternalOptionDialog(jContentPane,
                root, "Confirmation Change News",
                JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
                new ImageIcon(getClass().getResource(
                        Home.URL_IMAGES + "ModificaNews48.png")),
                options, options[1]);
        ...
        if (choice == JOptionPane.YES_OPTION) {
            try {
                ...
                expires.setDate(scadenza.getDate().getDate()
                        + DurataNews.getSelectedIndex() + 1);
                BeanNews newNews = new BeanNews(testoNews.getText(),
                        new Date(), expires, prSlider.getValue(), idNews);
                gestioneNews.modificaNews(newNews);
                tableModel.updateNews(newNews);
                JOptionPane.showInternalMessageDialog(jContentPane,
                        "The news has been successfully changed.",
                        "News changed!", JOptionPane.OK_OPTION,
                        new ImageIcon(getClass().getResource(
                                Home.URL_IMAGES + "Ok48.png")));
            } catch (Exception ex) {
                JLabel error = new JLabel(
                        "<html> <h2> Unable to communicate with the server eTour. </h2>"
                                + "<h3> <u> Change operation request cannot be completed. </u> </h3>"
                                + "<p> Please try again later. </p>"
                                + "<p> If the error persists, please contact technical support. </p>"
                                + "<p> We apologize for the inconvenience. </html>");
                ImageIcon err = new ImageIcon(getClass().getResource(
                        Home.URL_IMAGES + "Error48.png"));
                JOptionPane.showMessageDialog(jContentPane, error,
        ...
        expires.setDate(scadenza.getDate().getDate()
                + DurataNews.getSelectedIndex() + 1);
        BeanNews newNews = new BeanNews(testoNews.getText(), new Date(),
                expires, prSlider.getValue(), 33);
        try {
            ...
            if (ok) {
                ...
                JOptionPane.showInternalMessageDialog(jContentPane,
                        "The news is successfully inserted into the system.",
                        "New news!", JOptionPane.OK_OPTION,
                        new ImageIcon(getClass().getResource(
                                Home.URL_IMAGES + "NuovaNews48.png")));
            }
        } catch (RemoteException e) {
            JLabel error = new JLabel(
                    "<html> <h2> Unable to communicate with the server eTour. </h2>"
                            + "<h3> <u> Insertion operation request cannot be completed. </u> </h3>"
                            + "<p> Please try again later. </p>"
                            + "<p> If the error persists, please contact technical support. </p>"
                            + "<p> We apologize for the inconvenience. </html>");
            ImageIcon err = new ImageIcon(getClass().getResource(
...
private void caricaTabella() {
    ...
    try {
        ...
        JLabel error = new JLabel(
                "<html> <h2> Unable to communicate with the server eTour. </h2>"
                        + "<h3> <u> The list of News is not loaded. </u> </h3>"
                        + "<p> Please try again later. </p>"
                        + "<p> If the error persists, please contact technical support. </p>"
                        + "<p> We apologize for the inconvenience. </html>");
        ImageIcon err = new ImageIcon(getClass().getResource(
